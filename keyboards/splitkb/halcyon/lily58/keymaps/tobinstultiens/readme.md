...
Keymap naming and build target
------------------------------

This keymap is the `tobinstultiens` **C keymap** for `splitkb/halcyon/lily58/rev2`.

Compile it with:

```bash
qmk compile -kb splitkb/halcyon/lily58/rev2 -km tobinstultiens
```

There is also a separate JSON keymap named `tobinstultiens_hlc` in `keyboards/splitkb/halcyon/keymaps/tobinstultiens_hlc`.
That keymap was intentionally renamed to avoid keymap name collisions during QMK build resolution.
